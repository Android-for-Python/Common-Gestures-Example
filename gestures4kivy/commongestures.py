"""
    ...
    # Detects the common gestures for `scale`, `move`, `swipe`, `long press`,
        ...
        On the desktop it also detects `mouse wheel` and the touchpad
        equivalent `two finger move`.
...
class CommonGestures(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self._new_gesture()
        # Sensitivity
        self._DOUBLE_TAP_TIME = Config.getint(
            'postproc', 'double_tap_time') / 1000
    ...
    # Kivy Touch Events
        ...
        The on_touch_* callbacks have the required value because of
        collide_point but only within the scope of that touch callback.

        This is an issue for gestures with persistence,
        for example two touches.

...
from gestures4kivy.gestureboxlayout import GestureBoxLayout
from gestures4kivy.gesturecanvas import GestureCanvas
from gestures4kivy.gesturelabel import GestureLabel
from gestures4kivy.swipescreen import SwipeScreen
from gestures4kivy.zoomimage import ZoomImage
    ...
    def on_size(self, *args):
        ...
        text = 'FIRST SCREEN.\n\n' + \
            fill('Swipe to the left to pull the next screen from the right.',
                 COLS) + '\n\n' +\
            fill(
                ('On the screens after this one you can also swipe right '
        ...
        if not mobile:
            text += '\n\n' + \
                fill(
                    ('Mouse users hold the left button and '
                     'swipe the mouse.'), COLS) + '\n\n' + \
                fill(
                    ('Touch pad users the swipe begins with '
...
class Screen2(SwipeScreen):
    ...
    def on_size(self, *args):
        ...
        text = 'SECOND SCREEN.\n\n' + \
            fill(
                ('The gesture sensitive widget is grey. In the grey area '
            ...
            fill(
                ('A horizonal swipe anywhere in the screen changes the '
        ...
        if not mobile:
            text += '\n' +\
                fill(
                    ('Touch pad: like the "horizontal swipe", the '
...
class Screen3(SwipeScreen):
    ...
    def on_size(self, *args):
        ...
        text = 'THIRD SCREEN.\n\n' +\
            fill(
                ('The whole screen is sensitive to gestures '
                 'because in this case the gestures are bound to a Layout.'),
                COLS
            ) + '\n\n' +\
            fill(
                ('Try the same gestures as before, the results of the '
...
class Screen4(SwipeScreen):
    ...
    def on_size(self, *args):
        ...
        text = 'FOURTH SCREEN.\n\n' + \
            fill(
                ('Use "long press move" and "zoom" to change items on a '
                 'canvas.'), COLS) + '\n\n' + \
            fill(
                ('Long press on the red box, wait for visual feedback '
                 'then it can be dragged.'), COLS) + '\n' + \
            fill('Visual feedback is helpful with long press.', COLS) + \
            '\n\n' + \
            fill(
                ('Change the size of the box with a pinch/spread gesture '
                 'centered on the box.'), COLS)

        if not mobile:
            text += '\n' + \
                fill(
                    ('Touch pad and mouse users, place the cursor on the '
                     'box.'), COLS) + '\n' + \
...
class Screen5(SwipeScreen):
    ...
    def on_size(self, *args):
        ...
        text = 'LAST SCREEN.\n\n' + \
            fill(
                ('Zoom in using a spread gesture.Zoom out using a pinch '
                 'gesture.'), COLS) + '\n' + \
        ...
        if not mobile:
            text += '\n' + \
                fill(
                    ('Mouse users use "ctrl-scroll wheel" to zoom, '
